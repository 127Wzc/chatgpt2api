...
def _resolve_conversation_image_urls(
        backend: Any,
        conversation_id: str,
        file_ids: list[str],
    ...
    resolver = backend.resolve_conversation_image_urls
    try:
    ...
    kwargs: dict[str, Any] = {}
    if accepts_kwargs or "poll" in parameters:
        kwargs["poll"] = poll
    if poll_timeout_secs is not None and (accepts_kwargs or "poll_timeout_secs" in parameters):
        kwargs["poll_timeout_secs"] = poll_timeout_secs
    return resolver(conversation_id, file_ids, sediment_ids, **kwargs)
...
def stream_image_outputs(
    ...
    if message and not file_ids and not sediment_ids and last.get("blocked"):
        ...
        return
    should_poll_for_image = is_supported_image_model(request.model) or bool(request.images) or last.get("turn_use_case") == "image gen"
    ...
    try:
        image_urls = _resolve_conversation_image_urls(
            backend,
