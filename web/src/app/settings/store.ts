...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    image_pool_recovery_cooldown_secs: Number(config.image_pool_recovery_cooldown_secs || 10),
...
type SettingsStore = {
  ...
  setImagePoolRecoveryCooldownSecs: (value: string) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_pool_recovery_cooldown_secs: Math.max(1, Number(config.image_pool_recovery_cooldown_secs) || 10),
  ...
  setImagePoolRecoveryCooldownSecs: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_pool_recovery_cooldown_secs: value } } : {});
  },
